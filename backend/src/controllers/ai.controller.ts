...
export async function naturalSearch(req:Request,res:Response,next:NextFunction){try{const input=naturalSearchSchema.parse(req.body);const instruction=`Extract textile marketplace filters from this buyer request. Return a JSON object with ONLY the explicitly mentioned constraints. Allowed keys: search, category, gsmMin(number), gsmMax(number), composition, weaveType, stockType (MUST be "ready_stock" or "made_to_order"), certification. CRITICAL: Do NOT invent or guess any values. If a constraint is not explicitly requested, OMIT the key entirely. Do NOT put conversational words (like "set", "filter", "apply", "show", "gsm", "compare") into the "search" key; only use "search" for actual product keywords. If the user asks to compare multiple distinct materials (e.g. "compare nylon and cotton"), put those materials ONLY in the "search" key and YOU MUST PRESERVE the word "and" or commas between them (e.g. {"search": "nylon and cotton"}) so the system knows to search for both. DO NOT put them in "composition" or "category". Example 1: "show organic" -> {"search":"organic"}. Example 2: "cotton 200 gsm" -> {"composition":"cotton","gsmMin":200,"gsmMax":200}. Example 3: "set gsm 110" -> {"gsmMin":110,"gsmMax":110}. Request: ${input.query}`;const result:any=await converse("You are a precise JSON extraction service.",[{role:"user",content:instruction}]);const raw=result?.choices?.[0]?.message?.content??"{}";const matched=raw.match(/\{[\s\S]*\}/);let filters:Record<string,unknown>={};try{filters=JSON.parse(matched?.[0]??"{}");if(typeof filters.stockType==="string"){filters.stockType=filters.stockType.replace(/[-\s]/g,"_").toLowerCase();if(filters.stockType==="ready_to_order")filters.stockType="ready_stock";}}catch{}res.json({data:{filters}});}catch(error){next(error)}}
...
Allowed form keys: name, category, description, gsm, composition, weaveType, width, shrinkageRate (can include %), colorfastnessRating (can include /), colors, stockQty, stockType (MUST be exactly "ready_stock" or "made_to_order"), leadTimeDays, price, certifications, sustainabilityTags, tiers (MUST be a single comma-separated string formatted as "minQty:price", e.g. "50:15, 100:12").
CRITICAL RULES:
...
3. You MUST extract every field provided, including name, category, and description. However, if the user leaves them out, do NOT ask for them, because they can type them directly into the visual UI. Only ask for clarification if a provided value is ambiguous.
...
6. If the user asks to clear a specific field, output the field with an empty string as its value (e.g. {"description": ""}). If they ask to clear all fields, output ALL allowed form keys with an empty string.
7. Always output exactly this format: {"form": {"key": "value"}, "message": "Your reply here."}`;const messages:{role:"user"|"assistant";content:string}[]=[{role:"user",content:instruction},...input.messages.map(m=>({role:m.role,content:m.content}))];const result:any=await converse("You are a precise JSON extraction service.",messages);const raw=result?.choices?.[0]?.message?.content??"{}";console.log("LLM RAW OUTPUT:", raw);const matched=raw.match(/\{[\s\S]*\}/);let parsed:any={};try{parsed=JSON.parse(matched?.[0]??"{}");}catch{}
...
const extractedProfileSchema=z.object({businessName:z.union([z.string(),z.literal("")]).optional().catch(undefined),businessType:z.union([z.string(),z.literal("")]).optional().catch(undefined),email:z.union([z.string(),z.literal("")]).optional().catch(undefined),phone:z.union([z.string(),z.literal("")]).optional().catch(undefined),businessAddress:z.union([z.string(),z.literal("")]).optional().catch(undefined),operatingHours:z.union([z.string(),z.literal("")]).optional().catch(undefined),moq:numericString,productCategories:commaString,fabricTypesOffered:commaString}).strip();
export async function fillProfileForm(req:Request,res:Response,next:NextFunction){try{const input=fillFormSchema.parse(req.body);const instruction=`Extract business profile form fields from this supplier conversation. Return a JSON object with two keys: "form" (the explicitly mentioned valid fields) and "message" (your conversational reply).
Allowed form keys: businessName, businessType, email, phone, businessAddress, operatingHours, moq, productCategories, fabricTypesOffered.
CRITICAL RULES:
...
const rawForm=parsed.form||{};
const normalizedForm:any={};
const keyMap:Record<string,string>={businessname:'businessName',businesstype:'businessType',email:'email',phone:'phone',businessaddress:'businessAddress',operatinghours:'operatingHours',moq:'moq',productcategories:'productCategories',categories:'productCategories',fabrictypesoffered:'fabricTypesOffered',fabrics:'fabricTypesOffered',fabrictypes:'fabricTypesOffered'};
for(const [k,v] of Object.entries(rawForm)){
  const normK=k.toLowerCase().replace(/[^a-z0-9]/g,'');
  const mapped=keyMap[normK]||k;
  normalizedForm[mapped]=v;
}
const safeForm=extractedProfileSchema.safeParse(normalizedForm);
const validForm=safeForm.success?safeForm.data:{};
...
Allowed form keys: businessType, industry, productCategoriesInterest (also maps to "categories"), preferredFabricTypes (also maps to "fabrics"), typicalOrderQuantity (also maps to "quantity"), budgetRange (also maps to "budget"), additionalPreferences (also maps to "preferences").
CRITICAL RULES:
1. Do NOT invent or guess any values.
2. ALWAYS extract fields mentioned in the latest message, even if updating/overwriting. If the user uses shorthand (e.g. "budget" instead of "budgetRange", "categories" instead of "productCategoriesInterest"), YOU MUST map it and extract it.
3. Only ask for clarification if a provided value is ambiguous. Do NOT ask for missing info that the user did not provide.
4. If they ask to clear a specific field, output the field with an empty string as its value. If they ask to clear all fields, output ALL allowed form keys with an empty string.
5. For typicalOrderQuantity and budgetRange, extract ONLY the numbers and hyphens. Strip out any text like "metres" or "Rs".
