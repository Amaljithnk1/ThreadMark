...
Allowed form keys: name, category, description, gsm, composition, weaveType, width, shrinkageRate, colorfastnessRating, colors, stockQty, stockType ("ready_stock" or "made_to_order"), leadTimeDays, price, certifications, sustainabilityTags, tiers.
CRITICAL RULES:
1. Do NOT invent or guess any values (e.g. do NOT output "Unknown").
2. If the user provides garbage/invalid data for a field (e.g. a number for a text field, or invalid formatting), do NOT put it in "form". Instead, in your "message", explain the issue, provide a hint on what it should look like, and ask them to clarify.
3. If they are answering a previous clarification (e.g. "yeah its colorfastness"), use the conversation history to map their answer to the correct field in "form", and acknowledge it in "message".
4. If they just ask a general question, leave "form" empty and answer their question in "message".
5. Always output exactly this format: {"form": {"key": "value"}, "message": "Your reply here."}`;const messages=[{role:"user",content:instruction},...input.messages.map(m=>({role:m.role,content:m.content}))];const result:any=await converse("You are a precise JSON extraction service.",messages);const raw=result?.choices?.[0]?.message?.content??"{}";const matched=raw.match(/\{[\s\S]*\}/);let parsed:any={};try{parsed=JSON.parse(matched?.[0]??"{}");}catch{}
