...
6. Always output exactly this format: {"form": {"key": "value"}, "message": "Your reply here."}`;const messages:{role:"user"|"assistant";content:string}[]=[{role:"user",content:instruction},...input.messages.map(m=>({role:m.role,content:m.content}))];const result:any=await converse("You are a precise JSON extraction service.",messages);const raw=result?.choices?.[0]?.message?.content??"{}";console.log("LLM RAW OUTPUT:", raw);const matched=raw.match(/\{[\s\S]*\}/);let parsed:any={};try{parsed=JSON.parse(matched?.[0]??"{}");}catch{}
