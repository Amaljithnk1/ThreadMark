...
export async function getProductReviews(req: Request, res: Response, next: NextFunction) { try { const result = await query(`SELECT r.id, r.rating, r.comment, r.verified_purchase, r.created_at, COALESCE(bp.business_type, 'Verified Buyer') AS buyer_name FROM product_reviews r JOIN users u ON u.id = r.buyer_id LEFT JOIN buyer_profiles bp ON bp.user_id = u.id WHERE r.product_id = $1 ORDER BY r.created_at DESC`, [req.params.id]); res.json({ data: result.rows }); } catch (error) { next(error); } }
export async function checkReviewEligibility(req: Request, res: Response, next: NextFunction) { try { const result = await query(`SELECT 1 FROM order_items oi JOIN orders o ON o.id = oi.order_id WHERE o.buyer_id = $1 AND oi.product_id = $2 AND o.status = 'completed' LIMIT 1`, [req.auth!.userId, req.params.id]); res.json({ data: { eligible: result.rows.length > 0 } }); } catch (error) { next(error); } }
