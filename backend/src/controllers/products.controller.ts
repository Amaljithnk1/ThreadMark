...
const listingSchema = z.object({
  ...
  weaveType: z.string().trim().max(100).optional(), stockType: z.enum(["ready_stock", "made_to_order"]).optional().catch(undefined), certification: z.string().trim().max(100).optional(), sustainabilityTag: z.string().trim().max(100).optional(),
});
...
export async function getProductReviews(req: Request, res: Response, next: NextFunction) { try { const result = await query(`SELECT r.id, r.rating, r.comment, r.supplier_reply, r.supplier_replied_at, r.verified_purchase, r.created_at, u.username AS buyer_name FROM product_reviews r JOIN users u ON u.id = r.buyer_id WHERE r.product_id = $1 ORDER BY r.created_at DESC`, [req.params.id]); res.json({ data: result.rows }); } catch (error) { next(error); } }
